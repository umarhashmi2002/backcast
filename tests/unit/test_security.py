...
def test_valid_signature_accepted() -> None:
    ...
    assert verify_webhook(secret=SECRET, body=BODY, signature=sig, timestamp="1000", now=1010) is True


def test_tampered_body_rejected() -> None:
    sig = sign_payload(SECRET, BODY, 1000)
    assert verify_webhook(secret=SECRET, body=b"tampered", signature=sig, timestamp="1000", now=1010) is False


def test_wrong_secret_rejected() -> None:
    sig = sign_payload("other-secret", BODY, 1000)
    assert verify_webhook(secret=SECRET, body=BODY, signature=sig, timestamp="1000", now=1010) is False


def test_stale_timestamp_rejected() -> None:
    sig = sign_payload(SECRET, BODY, 1000)
    assert verify_webhook(secret=SECRET, body=BODY, signature=sig, timestamp="1000", now=1400) is False
