...
SECRET="$(aws secretsmanager get-secret-value --secret-id "$WEBHOOK_SECRET_ID" --query SecretString --output text 2>/dev/null)"
[[ -n "$SECRET" ]] || { echo "${R}could not read webhook secret ($WEBHOOK_SECRET_ID) — are AWS creds active?${X}" >&2; exit 2; }
...
if [[ -n "$INCIDENT_ID" ]]; then
  ...
  check "commander 200 + summary" "1" "(no incident_id from step 3)"
fi
...
TOTAL=$((PASS+FAIL))
echo
if [[ $FAIL -eq 0 ]]; then echo "${G}PASS ${PASS}/${TOTAL}${X}"; exit 0
else echo "${R}FAIL — ${PASS}/${TOTAL} passed, ${FAIL} failed${X}"; exit 1; fi
