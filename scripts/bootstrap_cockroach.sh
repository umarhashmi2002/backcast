...
if ccloud cluster list -o json | jq -e --arg n "$CLUSTER_NAME" '.[] | select(.name == $n)' >/dev/null; then
  ...
  ccloud cluster create BASIC "$CLUSTER_NAME" "$REGION" --cloud "$CLOUD" --wait -o json | jq '.'
fi
...
echo "==> Creating a SQL user"
echo "    ccloud cluster user create ${CLUSTER_NAME} <user> --password <password>"
echo "    (then grant it access to '${DB_NAME}')"

echo "==> Creating database '${DB_NAME}'"
echo "    ccloud has no inline-SQL flag; create it over the connection string:"
echo "      psql \"\$(ccloud cluster sql ${CLUSTER_NAME} --connection-url)\" -c 'CREATE DATABASE IF NOT EXISTS ${DB_NAME}'"
