...
**Project:** Retrace — agentic memory for an SRE Incident Commander on CockroachDB + AWS.
...
## Required: CockroachDB tools (≥ 2 — Retrace uses all four)
| Tool | Where |
|------|-------|
| Distributed Vector Indexing (C-SPANN) | `db/migrations/0001_init.sql` (vector indexes); `src/retrace/memory/evidence.py` (`<->` recall). |
...
## Required: AWS services (≥ 1 — Retrace uses several)
...
- ✅ Reproducible demo: `make db-up && uv run retrace demo`.
